...
into :mod:`app.services.solar.download_manager` for byte-accurate progress.
...
def export_hmi_vector_urls(
    *,
    ...
    """Resolve direct URLs for the hmi.B_720s vector segments.

    Every requested time step yields one URL per segment; the caller pairs
    them back up by record. The default quick/as-is path keeps the files
    Rice-compressed but header-less (HMI keywords live in DRMS, not the SUMS
    segment files); pass ``method="url", protocol="fits"`` for a staged export
    whose files carry the full keyword set (needed for exact axis transforms).
    """
    ...
    _logger.info(
        "JSOC vector export: series=%s method=%s protocol=%s recordset=%s",
        series, method, protocol, recordset,
    )
    try:
        request = client.export(recordset, method=method, protocol=protocol, email=email)
    except TypeError:
        request = client.export(recordset, method=method, protocol=protocol)
    except Exception as exc:
        raise JsocError(f"JSOC export failed: {exc}") from exc

    _wait_for_request(request, staged=(method != "url_quick"))
