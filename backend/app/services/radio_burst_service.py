...
Which model runs here is configuration, not a per-request choice:
``radio_burst_binary_model`` picks the binary classifier and
``radio_burst_classify_types`` decides whether each burst-positive file is also
labelled with a burst type. One model at a time keeps the alert stream (and the
scorecard derived from it) internally comparable; the on-demand Burst Detector
page is where models get compared side by side.
...
async def rebuild_radio_burst_events(db: AsyncSession) -> int:
    ...
    detections = await burst_positive_in_range(
        db, since, now, alert_min_probability(spec)
    )
