"""Canonical shock-analysis session normalizer — ported from the desktop app.

Binary-compatible with ``e-Callisto_FITS_Analyzer/src/Backend/analysis_session.py`` so
that a ``.efaproj`` written here restores the same Analyzer state in the desktop app and
vice-versa. Pure numpy; no Qt / I/O. Only ``normalize_session``, ``to_project_payload``,
``from_legacy_max_intensity`` and ``validate_session_for_source`` are used by the web
backend, but the full field tables are kept so the ``type_ii`` block round-trips even
though the web only produces Path A (single-band drift) results today.
"""
