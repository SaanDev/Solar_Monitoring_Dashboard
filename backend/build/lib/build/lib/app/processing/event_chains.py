...
def event_id(e: dict) -> str:
    ...
    The start is normalised to UTC-aware first, so the id does not depend on where
    the dict came from: a freshly derived event carries tzinfo, one read back from
    a backend that drops it (SQLite) does not, and both have to produce the same
    id — chain ids, alert ids and the sent-notification ledger are all keyed on it.
    """
    start = e["start_time"]
    if start.tzinfo is None:
        start = start.replace(tzinfo=timezone.utc)
    return f"{e['type']}:{start.isoformat()}"
